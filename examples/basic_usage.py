...
async def dshield_security_analysis():
    ...
    try:
        ...
        attack_report = data_processor.generate_attack_report(
            processed_events, 
            threat_intelligence
        )
        ...
        high_threat_ips = [ip for ip, ti in threat_intelligence.items() 
                          if ti.get('threat_level') == 'high']
        print(f"  High Threat IPs: {len(high_threat_ips)}")
        for ip in high_threat_ips[:5]:
            ti_data = threat_intelligence[ip]
            print(f"    {ip}: {ti_data.get('country', 'Unknown')} - Score: {ti_data.get('reputation_score', 'N/A')}")
    ...
    try:
        ...
        try:
            ...
            for i, attacker in enumerate(top_attackers[:5], 1):
                print(f"    {i}. {attacker.get('ip_address', 'N/A')}: {attacker.get('attack_count', 0)} attacks")
        except Exception as e:
