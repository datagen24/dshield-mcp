...
async def example_basic_anomaly_detection() -> None:
    ...
    try:
        ...
        if result["success"]:
            ...
            print(f"📊 Total anomalies detected: {result['anomaly_analysis']['summary']['total_anomalies_detected']}")
            print(f"🔍 Methods used: {', '.join(result['anomaly_analysis']['summary']['methods_applied'])}")
            print(f"⚙️ Sensitivity threshold: {result['anomaly_analysis']['summary']['sensitivity_threshold']}")
...
async def example_custom_anomaly_detection() -> None:
    ...
    try:
        ...
        if result["success"]:
            ...
            for method_name, method_data in result['anomaly_analysis']['anomalies_by_method'].items():
