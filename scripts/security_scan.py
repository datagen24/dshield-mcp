...
class MCPShieldScanner:
    ...
    def check_mcp_shield_installed(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["npx", "mcp-shield", "--help"], capture_output=True, text=True, timeout=10
            )
    ...
    def install_mcp_shield(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["npm", "install", "-g", "mcp-shield"], capture_output=True, text=True, timeout=300
            )
    ...
    def _generate_summary(self, output: str) -> dict[str, Any]:
        ...
        summary["vulnerable_servers"] = len(
            set(v["server"] for v in self.scan_results.get("vulnerabilities", []))
        )
    ...
    def generate_report(self, output_file: str | None = None) -> str:
        ...
        report.append(
            f"- Medium Risk Vulnerabilities: {summary.get('medium_risk_vulnerabilities', 0)}"
        )
...
def main():
    ...
    scanner = MCPShieldScanner(config_path=args.config_path, safe_list=safe_list)
    ...
    # Run scan
    results = scanner.scan(claude_api_key=args.claude_api_key, identify_as=args.identify_as)
