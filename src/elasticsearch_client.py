...
class ElasticsearchClient:
    ...
    async def query_dshield_events(
        ...
        try:
            ...
            logger.error(f"Error querying DShield events: {e!s}")
    ...
    async def execute_aggregation_query(
        ...
        try:
            ...
            logger.error(f"Error executing aggregation query: {e!s}")
            if self.error_handler:
                # Create appropriate error response based on exception type
                if isinstance(e, RequestError):
                    return {
    ...
    async def stream_dshield_events(
        ...
        try:
            ...
            logger.error(f"Error streaming DShield events: {e!s}")
    ...
    def log_unmapped_fields(self, source: dict[str, Any]) -> None:
        ...
        unmapped = [f for f in source if f not in mapped_fields]
        if unmapped:
    ...
    async def stream_dshield_events_with_session_context(
        ...
        try:
            ...
            logger.error(f"Error streaming DShield events with session context: {e!s}")
            raise
