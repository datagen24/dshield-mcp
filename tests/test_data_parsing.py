...
class TestElasticsearchClientFieldMapping:
    ...
    def test_extract_field_mapped_complex_nested(self):
        ...
        assert self.client._extract_field_mapped(source_data, "event_type") == "authentication_failure"
        ...
        assert self.client._extract_field_mapped(source_data, "description") == "Failed login attempt"
    ...
    def test_map_query_fields_with_nested_filters(self):
        ...
        assert mapped_filters["@timestamp"] == {"gte": "2024-01-01T00:00:00Z", "lte": "2024-01-01T23:59:59Z"}
            ...
            try:
                ...
                if not event['description']:
                    if http_method and http_status:
                        event['description'] = f"HTTP {http_method} request with status {http_status}"
                    elif http_method:
                        event['description'] = f"HTTP {http_method} request"
                    elif user_agent:
                        event['description'] = f"Request with user agent: {user_agent[:50]}..."
                    else:
                        event['description'] = f"{event['event_type']} event from {event['source_ip'] or 'unknown'} to {event['destination_ip'] or 'unknown'}"
...
class TestDataProcessingWorkflow:
    ...
    def test_end_to_end_data_processing(self, mock_get_config, mock_resolve_secrets, mock_user_config):
