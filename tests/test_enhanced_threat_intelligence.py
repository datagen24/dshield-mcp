...
class TestThreatIntelligenceManager:
    ...
    async def test_manager_initialization(self, mock_config):
        ...
        with (
            patch('src.threat_intelligence_manager.get_config', return_value=mock_config),
    ...
    async def test_enrich_ip_comprehensive_no_sources(self):
        ...
        with (
            patch(
                'src.threat_intelligence_manager.get_config',
                return_value={
                    "threat_intelligence": {
                        "sources": {"dshield": {"enabled": False}, "virustotal": {"enabled": False}}
                    }
                },
            ),
            patch(
                'src.threat_intelligence_manager.get_user_config',
                return_value=self.mock_user_config(),
            ),
        ):
            manager = ThreatIntelligenceManager()
...
class TestThreatIntelligenceModels:
    ...
    def test_threat_intelligence_result_creation(self):
        ...
        result = ThreatIntelligenceResult(
            ip_address="8.8.8.8", overall_threat_score=25.0, confidence_score=0.8
        )
    ...
    def test_domain_intelligence_creation(self):
        ...
        result = DomainIntelligence(domain="example.com", threat_score=30.0, reputation_score=70.0)
...
class TestThreatIntelligenceIntegration:
    ...
    async def manager(self) -> ThreatIntelligenceManager:
        ...
        # Clean up SQLite cache file after each test
        if (
            hasattr(manager, 'sqlite_cache_enabled')
            and manager.sqlite_cache_enabled
            and os.path.exists(manager.sqlite_cache_path)
        ):
            try:
                os.remove(manager.sqlite_cache_path)
            except Exception:
    ...
    async def test_cache_eviction(self):
        ...
        def mock_user_config():
            ...
            # Mock database path methods
            config.get_database_directory.return_value = "/tmp/test_db"
            config.get_cache_database_path.return_value = (
                "/tmp/test_db/test_enrichment_cache.sqlite3"
            )
        ...
        with (
            patch(
                'src.threat_intelligence_manager.get_config',
                return_value={"threat_intelligence": {"sources": {"dshield": {"enabled": True}}}},
            ),
            patch(
                'src.threat_intelligence_manager.get_user_config', return_value=mock_user_config()
            ),
        ):
            async with ThreatIntelligenceManager() as manager:
    ...
    async def test_elasticsearch_writeback_enabled(self):
        ...
        mock_config = {
            "threat_intelligence": {
                "sources": {"dshield": {"enabled": True, "rate_limit_requests_per_minute": 60}},
                "elasticsearch": {
                    "enabled": True,
        ...
        mock_user_config.get_cache_database_path.return_value = (
            "/tmp/test_db/test_enrichment_cache.sqlite3"
        )
    ...
    async def test_enhanced_correlation(self, manager):
        ...
        if len(result.threat_indicators) > 1:
            assert (
                result.threat_indicators[0]['confidence']
                >= result.threat_indicators[1]['confidence']
            )
...
class TestThreatIntelligenceIntegrationLive:
    ...
    @pytest.mark.skipif(
        ...
        reason="Set RUN_LIVE_INTEGRATION_TESTS=1 to run live integration tests.",
    )
...
class TestThreatIntelligenceIntegrationMocked:
    ...
    async def test_end_to_end_enrichment_and_writeback(self):
        ...
        with (
            ...
            mock_uc.get_cache_database_path.return_value = (
                "/tmp/test_db/test_enrichment_cache.sqlite3"
            )
