...
class TestOnePasswordSecretsIntegration:
    ...
    def test_config_loading_with_secrets_resolution(self):
        ...
        with patch('builtins.open', create=True) as mock_open, \
            ...
            assert config['dshield']['base_url'] == 'https://api.dshield.org'  # Non-secret unchanged
    ...
    def test_config_loading_with_nested_structures(self):
        ...
        mock_config_content = {
            'services': {
                ...
                'redis': {
                    'password': 'op://vault/item/redis_pass'
                }
            }
        }
        ...
        with patch('builtins.open', create=True) as mock_open, \
            ...
            assert config['services']['elasticsearch']['credentials']['username'] == 'resolved_value'
            assert config['services']['elasticsearch']['credentials']['password'] == 'resolved_value'
            assert config['services']['elasticsearch']['hosts'][0] == 'localhost'  # Non-secret unchanged
    ...
    def test_config_loading_with_op_cli_errors(self):
        ...
        with patch('builtins.open', create=True) as mock_open, \
            ...
            mock_open.return_value.__enter__.return_value.read.return_value = "mock content"
            mock_yaml_load.return_value = mock_config_content
            mock_run.side_effect = subprocess.CalledProcessError(1, "op", stderr="Authentication failed")
