...
class TestElasticsearchCircuitBreakerIntegration:
    ...
    def test_circuit_breaker_status_retrieval(
        self, elasticsearch_client: ElasticsearchClient
    ) -> None:
...
class TestElasticsearchCircuitBreakerInOperations:
    ...
    @pytest.mark.asyncio
    async def test_query_events_with_circuit_breaker_closed(
        self, elasticsearch_client: ElasticsearchClient
    ) -> None:
        ...
        with patch.object(elasticsearch_client, 'get_available_indices') as mock_get_indices:
            ...
            events, total_count, pagination_info = await elasticsearch_client.query_dshield_events(
                time_range_hours=24, page=1, page_size=100
            )
    ...
    @pytest.mark.asyncio
    async def test_query_events_with_circuit_breaker_open(
        self, elasticsearch_client: ElasticsearchClient
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_stream_events_with_circuit_breaker_closed(
        self, elasticsearch_client: ElasticsearchClient
    ) -> None:
        ...
        with patch.object(elasticsearch_client, 'get_available_indices') as mock_get_indices:
            ...
            events, total_count, last_event_id = await elasticsearch_client.stream_dshield_events(
                time_range_hours=24, chunk_size=500
            )
    ...
    @pytest.mark.asyncio
    async def test_stream_events_with_circuit_breaker_open(
        self, elasticsearch_client: ElasticsearchClient
    ) -> None:
