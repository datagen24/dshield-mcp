...
class TestSecurityValidator:
    ...
    def test_validate_tool_description_hidden_instructions(self, validator: SecurityValidator) -> None:
    ...
    def test_validate_tool_description_sensitive_file_access(self, validator: SecurityValidator) -> None:
        ...
        sensitive_issue = next(issue for issue in issues if issue.issue_type == "sensitive_file_access")
    ...
    def test_validate_tool_description_excessive_length(self, validator: SecurityValidator) -> None:
        ...
        length_issue = next(issue for issue in issues if issue.issue_type == "excessive_description_length")
    ...
    def test_validate_tool_schema_suspicious_parameters(self, validator: SecurityValidator) -> None:
        ...
        suspicious_params = [issue.details["parameter_name"] for issue in issues if issue.issue_type == "suspicious_parameter"]
    ...
    def test_validate_tool_schema_permissive_parameters(self, validator: SecurityValidator) -> None:
        ...
        permissive_issues = [issue for issue in issues if issue.issue_type == "permissive_parameter"]
        assert len(permissive_issues) >= 1
    
    def test_validate_tool_schema_missing_required_fields(self, validator: SecurityValidator) -> None:
        ...
        missing_field_issue = next(issue for issue in issues if issue.issue_type == "missing_required_field")
    ...
    def test_validate_tool_arguments_sensitive_data(self, validator: SecurityValidator) -> None:
        ...
        sensitive_issues = [issue for issue in issues if issue.issue_type == "sensitive_data_in_arguments"]
    ...
    def test_validate_tool_arguments_path_traversal(self, validator: SecurityValidator) -> None:
        ...
        traversal_issues = [issue for issue in issues if issue.issue_type == "potential_path_traversal"]
        assert len(traversal_issues) >= 1
    
    def test_validate_server_configuration_hardcoded_credentials(self, validator: SecurityValidator) -> None:
        """Test detection of hardcoded credentials in server configuration."""
        config = {
            "command": "python",
            "args": ["server.py"],
            "env": {
                "ELASTICSEARCH_PASSWORD": "hardcoded_password",
        ...
        credential_issues = [issue for issue in issues if issue.issue_type == "hardcoded_credentials"]
        assert len(credential_issues) >= 1
    
    def test_validate_server_configuration_insecure_commands(self, validator: SecurityValidator) -> None:
        ...
        command_issue = next(issue for issue in issues if issue.issue_type == "insecure_command_execution")
    ...
    def test_get_security_summary_with_issues(self, validator: SecurityValidator) -> None:
        ...
        validator.validate_tool_schema("testTool", {
...
class TestIntegration:
    ...
    def test_real_world_tool_validation(self) -> None:
        ...
        description_issues = validator.validate_tool_description("calculateSum", malicious_description)
        ...
        critical_issues = [issue for issue in description_issues if issue.severity == SecurityRiskLevel.HIGH]
        assert len(critical_issues) >= 1
        
        # Check for suspicious parameters
        suspicious_params = [issue for issue in schema_issues if issue.issue_type == "suspicious_parameter"]
    ...
    def test_clean_tool_validation(self) -> None:
        ...
        clean_description = "Query DShield events from Elasticsearch SIEM with enhanced pagination support"
        ...
        description_issues = validator.validate_tool_description("query_dshield_events", clean_description)
