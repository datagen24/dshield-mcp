...
class TestStreaming:
    ...
    async def mock_es_client(self):
        ...
        with patch('src.elasticsearch_client.ElasticsearchClient') as mock_class:
            ...
            client.stream_dshield_events_with_session_context = AsyncMock(return_value=([], 0, None, {}))
    ...
    async def test_streaming_with_field_selection(self, mock_es_client):
        ...
        mock_events = [
            {"@timestamp": "2024-01-01T10:00:00Z", "source_ip": "192.168.1.100", "event.category": "network"},
            {"@timestamp": "2024-01-01T10:01:00Z", "source_ip": "192.168.1.101", "event.category": "network"}
        ]
    ...
    async def test_streaming_error_handling(self, mock_es_client):
        ...
        with pytest.raises(Exception, match="Connection failed"):
            await mock_es_client.stream_dshield_events(
                time_range_hours=24,
                chunk_size=10
            )
...
class TestSmartChunking:
    ...
    async def test_basic_session_chunking(self, mock_es_client):
        ...
        mock_es_client.stream_dshield_events_with_session_context.return_value = (
            mock_events, 100, "session_stream_123", mock_session_context
        )
        
        events, total_count, next_stream_id, session_context = await mock_es_client.stream_dshield_events_with_session_context(
            time_range_hours=1,
            chunk_size=100
        )
        ...
        assert session_context["performance_metrics"]["query_time_ms"] == 150, "Should have performance metrics"
    ...
    async def test_custom_session_fields(self, mock_es_client):
        ...
        mock_es_client.stream_dshield_events_with_session_context.return_value = (
            mock_events, 50, "stream_456", mock_session_context
        )
        ...
        assert session_context["session_fields"] == ["source.ip", "destination.ip"], "Should use custom session fields"
    ...
    async def test_session_summaries(self, mock_es_client):
        ...
        mock_es_client.stream_dshield_events_with_session_context.return_value = (
            [], 200, "stream_789", mock_session_context
        )
    ...
    async def test_filtered_session_chunking(self, mock_es_client):
        ...
        mock_es_client.stream_dshield_events_with_session_context.return_value = (
            mock_events, 25, "filtered_stream", mock_session_context
        )
        ...
        assert total_count == 25, "Total count should be 25"
        assert session_context["sessions_in_chunk"] == 1, "Should have 1 session"
        assert session_context["performance_metrics"]["query_time_ms"] == 200, "Should have performance metrics"
    
    @pytest.mark.asyncio
    async def test_field_selection_with_session_chunking(self, mock_es_client):
        """Test field selection with session chunking."""
        mock_events = [
            {"@timestamp": "2024-01-01T10:00:00Z", "source.ip": "192.168.1.100", "destination.ip": "10.0.0.1", "event.type": "network"}
        ]
        ...
        mock_es_client.stream_dshield_events_with_session_context.return_value = (
            mock_events, 50, "field_stream", mock_session_context
        )
        ...
        assert len(events) == 1, "Should return 1 event"
        assert session_context["sessions_in_chunk"] == 1, "Should have 1 session"
        assert session_context["performance_metrics"]["query_time_ms"] == 100, "Should have performance metrics"
    ...
    async def test_session_gap_configuration(self, mock_es_client):
        ...
        mock_es_client.stream_dshield_events_with_session_context.return_value = (
            [], 100, "gap_stream", mock_session_context
        )
    ...
    async def test_performance_comparison(self, mock_es_client):
        ...
        mock_es_client.stream_dshield_events = AsyncMock(return_value=([{"id": "1"}], 100, "regular_stream"))
        ...
        mock_es_client.stream_dshield_events_with_session_context = AsyncMock(return_value=(
            [{"id": "1"}], 100, "session_stream", mock_session_context
        ))
        ...
        events_session, _, _, session_context = await mock_es_client.stream_dshield_events_with_session_context(
            time_range_hours=1,
            chunk_size=100
        )
        ...
        assert session_context["performance_metrics"]["sessions_processed"] == 2, "Should process 2 sessions"
        assert session_context["performance_metrics"]["query_time_ms"] == 150, "Should have performance metrics"
    ...
    async def test_session_chunking_error_handling(self, mock_es_client):
        ...
        # Mock error
        mock_es_client.stream_dshield_events_with_session_context.side_effect = Exception("Session processing failed")
