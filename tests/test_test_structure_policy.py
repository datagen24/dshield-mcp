...
class TestTestStructurePolicy:
    ...
    def test_no_legacy_test_files(self) -> None:
        ...
        assert not legacy_files, f"Found legacy test files that should be consolidated: {legacy_files}"
    ...
    def test_one_test_file_per_module(self) -> None:
        ...
        src_modules = [
            p for p in glob.glob("src/**/*.py", recursive=True) 
        ...
        for src_path in src_modules:
            ...
            expected_test = f"tests/{module_path.rsplit('/', 1)[0]}/test_{module_path.rsplit('/', 1)[-1]}.py"
        ...
        if missing_tests:
            missing_msg = "Missing test files:\n" + "\n".join(f"  {src} -> {test}" for src, test in missing_tests)
            print(missing_msg)
        ...
        filtered_missing = [
            (src, test) for src, test in missing_tests 
    ...
    def test_test_file_naming_convention(self) -> None:
        ...
        for test_file in test_files:
            ...
            if any(pattern in module_part for pattern in ["_enhanced", "_simple", "_old", "_clean", "_focused"]):
                invalid_names.append(test_file)
    ...
    def test_no_orphaned_test_files(self) -> None:
        ...
        test_files = glob.glob("tests/**/test_*.py", recursive=True)
        src_modules = {
            os.path.basename(p)[:-3] for p in glob.glob("src/**/*.py", recursive=True)
            if not p.endswith("__init__.py")
        }
        ...
        filtered_orphans = [
            test for test in orphaned_tests 
            if os.path.basename(test) not in allowed_orphans
        ]
